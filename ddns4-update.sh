...
#
# Usage:
#   ddns4-update.sh                          # use default config
#   ddns4-update.sh --config /path/to/conf   # custom config
...
if [[ -z "$CONFIG_FILE" ]]; then
    for candidate in \
        "$HOME/.config/macos-ddns6/ddns4.conf" \
        "/etc/macos-ddns6/ddns4.conf"; do
        if [[ -f "$candidate" ]]; then
...
if [[ -z "$CONFIG_FILE" || ! -f "$CONFIG_FILE" ]]; then
    echo "ERROR: No config file found. Copy ddns4.conf.example to ~/.config/macos-ddns6/ddns4.conf" >&2
    exit 1
fi
...
DNS_TTL="${DNS_TTL:-300}"
LOG_TAG="ddns4"
CACHE_FILE="/tmp/ddns4-update.cache"
...
log "Detected IPv4 address: ${ADDR} (previous: ${CACHED:-(none)})"
